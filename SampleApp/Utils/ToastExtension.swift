...
let HRToastVerticalMargin   : CGFloat  =   12.0
...
extension UIView {
    ...
    fileprivate func makeToastActivity(position pos: AnyObject, message msg: String = "") {
        ...
        if FullScreen {
            ...
            activityView = UIView(frame: CGRect(x: 0, y: 0, width: HRToastActivityWidth, height: HRToastActivityHeight))
    ...
    fileprivate func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercased == HRToastPositionTop {
                return CGPoint(x: viewSize.width/2, y: toastSize.height/2 + 4 * HRToastVerticalMargin)
